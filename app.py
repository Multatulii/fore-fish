...
def main():       
    ...
    input_ikan = st.selectbox('Jenis Ikan',("Udang","Lobster","Nila","Tengiri","Manyung","Gulamah","Lisong","Tongkol")) 
    input_months = st.slider('Number of months that will be predicted', min_value=0, max_value=6, step=1)
    ...
    if st.button("Predict"): 
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data2.iloc[0:48].index, y=data2[input_ikan].iloc[0:48], name='Data Actual'))
        if input_ikan != "Manyung":
            fig.add_trace(go.Scatter(x=data2.iloc[48:].index, y=data2["predicted_mean"].iloc[48:], name='Forecasts'))
        else:
            fig.add_trace(go.Scatter(x=data2.iloc[48:].index, y=data2[data2.columns[0]].iloc[48:], name='Forecasts'))
